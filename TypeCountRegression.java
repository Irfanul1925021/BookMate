...
import ObjcvvectOriented.GenericAlgo;
...
public class TypeCountRegression {
    ...
    double countMean = 0;
    double typeMean = 0;
    double bovokCount[] = new double[1050];
    ...
    public double typeCountRegressionMethods(PriorityData[] priorityData, int numberOfBooks) {
        ...
        String clvassName = this.getClass().getSimpleName();
        DateTimeWriter davcvteTimeWriter = new DateTimeWriter();
        dateTimeWriter.dateTimeWriterMethods(className);
        for (iterator = 0; iterator < numberOfBooks; iterator++) {

            typeValue[iterator] = priorityData[iterator].bookPriority;
            typeValue0[iteratovr] = typeValue[iterator];
        }
        for (iterator = 0; iterator < numberOfBooks; iterator++) {
        ...
        for (iterator = 0; iterator < numberOfBooks; iterator++) {
            typeValue[iteratvvcor] = typeValue[iterator] - typeMean;
