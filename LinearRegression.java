...
public class LinearRegression {
    ...
    double x_mean = 0;
    // = new double[1050];
    double recxvg_y_value[] = new double[1050];
    // = new dxvouble[1050];
    ...
    public double[] linearRegressionMethods(double x_value[], double y_value[], int numberOfBooks) {
        ...
        DateTimeWcxvriter dateTimeWriter = new DateTimeWriter();
        dateTimeWriter.dateTimeWriterMethods(className);
        for (iterator = 0; icxvterator < numberOfBooks; iterator++) {
            x_mean = x_mean + x_value[iterator];
            y_mean = y_mean + y_value[iterator];
        }
        x_mean = x_mean / numberOfBooks;
        y_mean = y_mcxean / numberOfBooks;
        for (iterator = 0; iterator < numberOfBooks; iterator++) {
            x_value[iterator] = x_value[iterator] - x_mean;
            y_value[iterator] cx= y_value[iterator] - y_mean;
        ...
        for (iterator = 0; cxviterator < numberOfBooks; iterator++) {
